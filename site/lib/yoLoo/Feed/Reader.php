    ...
    function feed_start_element($p, $element, &$attrs) {
        ...
        if ( !empty($this->incontent) )
        {
                if ((count($this->incontent) > 1) or !$this->exclude_top) {
                      ...
                      $attrs_str = join(' ',
                          array_map('\\yoLoo\\Feed\\map_attrs',
                          array_keys($attrs),
                          array_values($attrs) )
                        );
...
?>
Name: <?=$item['title']?><br />
Hïjd: <?=$item['media']['player@height']?><br />
Bred: <?=$item['media']['player@width']?><br />
Fil : <?=$item['media']['player@url']?><br />
<?
$a = array_filter(
        array_map(function($v){
            $q = strip_tags(str_ireplace('</dt><dd>','=>',$v));
            $r = explode('=>',$q);
            return array($r[0]=>$r[1]);
        }, explode('</dd>',$item['summary'])),
        function ($v){
            return in_array(strtolower(key($v)), array('tag','slug','leaderboards'));
        }
    );
...
foreach($a as $b){$c[] = $b;}
var_dump(($c));
//<embed src="http://static.jesper.nu/games/motor/Truck_Launch_Maniac.swf" width="640" height="480"></embed>
