 ...
 */
class Base extends \yoLoo\Base implements \yoLoo\Interfaces\Factory
{
    /**
     * Singleton instances.
     *
     ...
     * @var array
     */
   // protected $_yoLoo_SessionHandler_Base = array();
     ...
     *
     * @return yoLoo\SessionHandler\Base  The newly created concrete instance.
     * @throws yoLoo\Exception
     */
    static public function factory($params = array())
    {
        if(!isset ($params['driver']))
            throw new \yoLoo\Exception('no driver');
        $driver = basename(strtolower($params['driver']));
        ...
        $class = '\\yoLoo\\SessionHandler\\' . ucfirst($driver);

        if (class_exists($class)) {
            return new $class(array_merge($params, $persistent_params));
        }

        throw new yoLoo\Exception('Driver "' . $driver . '" not found.');
    }
    ...
    public static function registerHandler(Base $handler)
    {
...
//                Horde::logMessage('Session data unchanged (id = ' . $id . ')', __FILE__, __LINE__, PEAR_LOG_DEBUG);
    ...
    {
        throw new yoLoo\Exception("Not supported.");
    }
