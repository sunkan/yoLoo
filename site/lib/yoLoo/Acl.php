...
class Acl extends \yoLoo\Base
{
    ...
    public function addRole(Acl\Role\IRole $role, array $parents = array())
    {
        if ($this->hasRole($role))
        {
            throw new Acl\Exception('Role already exist!');
        }
        if(count($parents))
        {
            $parentObjs = array();
    ...
    public function getRole($role, $add=true)
    {
        ...
        if (($role instanceof Acl\Role\IRole) && $add)
        {
            $this->addRole($role);
            return $role;
        }
    ...
    public function isAllowed($role, $section=null, $action=null, $object=null)
    {
         ...
         *
         */
        if (($flag=$this->isOwner($roleObj, $object)))
        {
